...
- The Intel RAPL powercap interface exposes live `psys`, `package-0`, `core`, and `uncore` domains. `psys` is the platform-wide machine-health reading; `package-0` remains a separate CPU diagnostic, and overlapping child domains are never added together. The energy attributes default to root-only on kernel 7.0, which also rejects filesystem ACLs on those attributes. A fixed sudo rule therefore lets user `leo` invoke only the root-owned `big-red-powercap-read` helper with no arguments. The helper performs ordinary reads and emits only the allowlisted domain name, energy, and maximum range for `psys` and one `package-0`; it changes no counters and installs no resident service.
...
The GPU-passthrough Windows VM also has a private Sunshine/Moonlight path. Sunshine and Tailscale run as automatic Windows services. Air Blue's paired **Big Red Windows** tile targets the Ubuntu host's direct Tailscale path; a boot-persistent nftables service forwards only Sunshine's gameplay ports from `tailscale0` to the guest and excludes the Web UI. The tile launches the Windows desktop directly with the tested 1280x800, 30 FPS, 4 Mbps H.264 profile. Sunshine credentials and Moonlight pairing material remain only in local credential stores. See [`BIG_RED_WINDOWS_MOONLIGHT.md`](BIG_RED_WINDOWS_MOONLIGHT.md) for the reproducible, non-secret setup and recovery checks.
...
The internal display runs its native 3072x1920 mode at 165 Hz and 150% scale, starts each login at raw backlight 150/496 (about 30%), dims when idle, and powers off after 10 minutes. Local brightness keys can raise or lower it after login. Local input wakes the display immediately. Blanking does not stop ChatGPT, SSH, Tailscale, builds, or other background work; automatic suspend and screen locking remain disabled. A live desktop capture remained fully readable while the physical backlight reported blank, confirming that desktop-control routes can still inspect the session.

GNOME's native 10-minute timeout remains the primary policy. On 2026-09-07, a live session retained the correct `idle-delay=600` setting but failed to act after more than 11 hours of Mutter-reported inactivity, with no screen-idle inhibitor. The user timer `big-red-panel-idle-blank.timer` is therefore a bounded fail-safe: once per minute it reads Mutter's own idle time, honors GNOME screen-idle inhibitors, and requests `PowerSaveMode=3` only when the panel is still on after 10 idle minutes. It never synthesizes input, locks, or suspends. Refresh it with `scripts/install-big-red-panel-idle-blank`; local input remains Mutter's normal wake path.

Install or refresh the read-only RAPL access after cloning this repository with `scripts/install-big-red-powercap-telemetry`. Remove `/etc/sudoers.d/big-red-powercap-telemetry` and `/usr/local/sbin/big-red-powercap-read` to revoke it. The installer also removes the superseded udev/ACL attempt, which Big Red's sysfs correctly rejected before any reporter update.
