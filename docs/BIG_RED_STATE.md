...
- The Intel RAPL powercap interface exposes live `psys`, `package-0`, `core`, and `uncore` domains. `psys` is the platform-wide machine-health reading; `package-0` remains a separate CPU diagnostic, and overlapping child domains are never added together. The energy attributes default to root-only on kernel 7.0, so `90-big-red-powercap-telemetry.rules` grants user `leo` read-only ACL access to only the `psys` and `package-0` counters through a bounded helper. It changes no counters and installs no resident service.
...
Install or refresh the read-only RAPL access after cloning this repository with `scripts/install-big-red-powercap-telemetry`. The installer reloads udev rules and applies the same ACL to already-present named domains. Remove `/etc/udev/rules.d/90-big-red-powercap-telemetry.rules` and `/usr/local/sbin/big-red-powercap-access`, reload udev rules, and reboot to return to the kernel's default counter permissions.
